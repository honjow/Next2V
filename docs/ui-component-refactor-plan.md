...
### 2026-05-18 Lane 9 第七批

状态：PASS

范围：

- 拆 `Index.pm()` 导航目的地参数读取。
- 只集中 `TopicDetail`、`ImagePreview`、`NodeTopicList`、`ReplyEditor`、`UserProfile/UserTopics/UserReplies` 的 route 参数解析和默认值规则。
- 不移动页面构造、titleBar 绑定、导航栈 push、AppStorage 命令、副作用动作和任何站内写操作。

变更：

- `IndexRouteCoordinator` 新增 typed route 参数解析 helper，统一 number/string 参数默认值。
- `Index.pm()` 移除重复的 `Record<string, Object>` inline cast，页面分支继续只负责创建对应 destination。
- `ReplyEditor` 参数解析已集中，但实机 QA 不自动提交回复；回复/发帖/举报仍保留给用户指定内容或人工验证。
...
- `bash dev.sh --build-only` 通过。
- 已安装到 `192.168.50.237:12345` 并完成实机 QA。
- 证据目录：`.hermes-artifacts/20260518-1846-index-route-param-qa/`。
- 实机路径覆盖：`https://www.v2ex.com/t/1212580#reply117` 进入帖子详情；`https://www.v2ex.com/search?q=ArkTS` 进入搜索页并填入 `ArkTS`；`https://www.v2ex.com/go/programmer` 进入 `programmer` 节点页；`https://www.v2ex.com/member/Livid` 进入用户主页。

后续入口：

1. `Index.pm()` 后续如继续拆页面 destination 构造，应按页面族分批，避免一次触碰所有 titleBar 和 destination lifecycle。
2. `ReplyEditor`、发帖、举报等真实写操作路径只做静态拆分准备；实机提交必须由用户指定内容或用户亲自验证。

### 2026-05-18 Lane 9 第八批
...
- 拆设置存储页纯文案和状态格式规则。
- 统一设置首页“存储”行与存储页“离线缓存”行的缓存统计 subtitle。
- 不移动 `CacheSettings.clear`、`LocalDataSettings.clearAll`、`CacheDeviceQaSeed` 执行、Preferences、文件缓存或任何站内写操作。

变更：

- 新增 `StorageSettingsCoordinator`，集中缓存统计 subtitle、清理确认框文案、清理/读取/seed 成功失败文案。
- `SettingsPage` 的存储行改用 `StorageSettingsCoordinator.cacheSubtitle`，避免和存储详情页重复规则。
- `StorageSettingsPage` 继续负责 AlertDialog 展示、本地清理调用、Seed 调用和状态赋值。
...
- `bash dev.sh --build-only` 通过。
- 已安装到 `192.168.50.237:12345` 并完成实机 QA。
- 证据目录：`.hermes-artifacts/20260518-1851-storage-settings-coordinator-qa/`。
- 实机路径覆盖：首页 -> 我的 -> 设置 -> 滚动到“存储”行，显示 `列表 3 组 · 详情 1 篇 · 更新于 2026-05-18 18:51`；进入存储页，显示离线缓存、刷新缓存状态、清理缓存和 QA Seed 列表；点击“清理缓存”只验证确认框文案和按钮，未点击红色确认按钮。

后续入口：

1. 存储清理和 QA seed 是本地破坏性/写文件操作，后续如要改行为应单独 lane。
2. 设置页剩余可拆项主要是设置保存 adapter，但会触及 Preferences 和 WebCookie 同步，应排在更高风险批次。

### 2026-05-18 Lane 9 第九批

状态：PASS

范围：

- 继续拆 `Index.pm()` 的只读 destination 构造。
- 继续拆 `AccountPage` 只读展示文案和本地显示判断。
- 拆 `SettingsPage` 保存 adapter，集中已有 Preferences/WebCookie/MotionHand 保存调用。
- 不移动回复、发帖、举报、退出登录、清除授权、缓存清理、每日签到等真实写操作或破坏性操作。

变更：

- `IndexRouteCoordinator` 新增 ArkTS 兼容的 read-only destination descriptor；`Index.pm()` 把 `TopicDetail`、`ImagePreview`、`NodeTopicList`、`UserProfile/UserTopics/UserReplies` 交给只读 destination builder，`ReplyEditor` 和其他写/编辑入口仍留在原分支。
- `AccountPageCoordinator` 新增账号页 section/row 文案和简单显示判断；`AccountPage` 继续负责登录、登出、Token 清除、Cookie、网络、浏览器打开和本地内容加载。
- 新增 `SettingsSaveCoordinator`，集中 API 域名、媒体、自动签到、主题、回复显示/样式/按钮对齐、Base64、阅读位置等保存调用；`SettingsPage` 继续负责 UI 状态赋值、菜单和页面结构。

验证：

- read-only review gate：`git diff --check` 通过，`bash dev.sh --build-only` 通过。
- review 证据目录：`/tmp/v2next-review-20260518-1912-lanes-1-4-5/`。
- independent device QA gate：已安装到 `192.168.50.237:12345` 并完成实机 QA。
- QA 证据目录：`.hermes-artifacts/20260518-1915-lanes-1-4-5-qa/`。
- 实机路径覆盖：`https://www.v2ex.com/t/1212580#reply117`、`https://www.v2ex.com/go/programmer`、`https://www.v2ex.com/member/Livid`；我的/账号页展示；设置页顶部设置行和滚动后的存储行。
- QA 未执行站内写操作、登出、清除授权、清理缓存、回复、发帖、举报或每日签到。

后续入口：

1. `Index.pm()` 剩余大分支可继续按设置/账号/本地内容页面族拆，但需要继续覆盖导航 QA。
2. `SearchPage`、`NotificationPage` 的网络/分页状态机仍是中高风险专项。
3. 真实站内写操作和本地破坏性操作继续排后，必须由用户指定内容或亲自实机验证。

### 2026-05-18 高优先级技术债第二批

状态：PASS

范围：

- 收口 `ApiService` V1 公开接口中的字符串 query 参数编码。
- 只处理只读请求：按用户名获取主题、按节点名获取主题、节点详情、用户信息。
- 不移动站内写操作、Cookie 请求、登录、通知删除或本地破坏性操作。

变更：

- 新增 `ApiService.queryParam`，统一对 query key/value 做 `encodeURIComponent`，并 trim 字符串值。
- `getTopicsByUsername`、`getTopicsByNode`、`getNodeByName`、`getMember` 改用统一 helper，避免特殊字符、空格、`&`、`#` 等破坏请求语义。
- `test_network_retry_security_static.mjs` 增加防回归断言，禁止这些路径重新出现裸 query 字符串插值。
...
- `bash dev.sh --build-only` 通过。
- 已安装到 `192.168.50.237:12345` 并完成实机 QA。
- 证据目录：`.hermes-artifacts/20260518-1934-query-encoding-network-qa/`。
- 实机路径覆盖：`https://www.v2ex.com/go/programmer` 显示程序员节点详情和主题列表；`https://www.v2ex.com/member/Livid` 显示 Livid 用户资料和主题/回复列表。
- QA 未执行回复、发帖、举报、感谢、收藏、删除通知、清授权、登出、清理缓存或每日签到。

后续入口：

1. `ApiService` 写操作请求 helper 的异常边界仍是高风险项，必须单独 lane，并由用户指定内容或用户亲自验证真实写操作。
2. `NotificationPage` 网络/分页状态机、`SearchPage` 远端搜索状态机仍属中风险专项。
3. PAT/Cookie 安全存储迁移涉及登录态和凭据边界，继续排后，不做自主移动。

### 2026-05-18 Index.pm destination 收口

状态：PASS

范围：

- 彻底移除 `Index.pm()` 内按 route name 手写的 destination 分支链。
- 覆盖只读、设置、账号、本地内容、登录、搜索、图片预览、回复编辑和发帖草稿入口。
- 不移动 `NavPathStack` push、AppStorage 命令发送、标题栏 action 执行、站内写操作或页面内部业务逻辑。

变更：

- `IndexRouteCoordinator` 新增完整 `IndexDestinationDescriptor`，集中 route name -> destination family、标题文案和页面参数解析。
- `Index.pm()` 收敛为单行 descriptor 分发：`this.destination(IndexRouteCoordinator.destination(name, param))`。
- `Index` 保留实际 ArkUI 页面组件条件渲染，但共享单个 `HdsNavDestination` 外壳；标准标题栏页面由 `STANDARD_TITLE_BAR_FAMILIES` 字典维护，特殊标题栏、菜单和 `onShown` 进入普通 helper。
- 新增 `scripts/test_index_destination_contract.mjs`，防止 `Index.pm()` 重新出现 route 分支或直接拥有 `HdsNavDestination`。
- 由于 ArkUI `@Builder` 不支持把组件标签作为普通字典值动态实例化，页面组件本体仍需要显式条件块；当前收敛点是 route/标题栏/外壳字典化，避免 wrapper builder 和 `else if` 链重复维护。

验证：

- `node scripts/test_index_destination_contract.mjs` 通过。
- `node scripts/test_network_retry_security_static.mjs` 通过。
- `git diff --check` 通过。
- `bash dev.sh --build-only` 通过。
- 已安装到 `192.168.50.237:12345` 并完成实机 QA。
- 证据目录：`.hermes-artifacts/20260518-1952-index-destination-qa/`。
- 实机路径覆盖：`https://www.v2ex.com/t/1212580#reply117` 进入主题详情；`https://www.v2ex.com/go/programmer` 进入节点页；`https://www.v2ex.com/member/Livid` 进入用户页；`https://www.v2ex.com/search?q=ArkTS` 进入搜索页；首页 -> 我的 -> 设置进入设置页。
- 字典化 follow-up 已再次通过构建和实机 QA；证据目录：`.hermes-artifacts/20260518-2008-index-destination-dictionary-qa/`，覆盖同一组只读 deep link 和设置入口。
- QA 未执行回复、发帖、举报、感谢、收藏、删除通知、清授权、登出、清理缓存或每日签到。

后续入口：

1. `Index` 剩余复杂度主要在标题栏 action/menu 和 AppStorage command 发送；这些已经不属于 `pm()` destination 构造问题。
2. 若继续拆 `Index`，应按标题栏 action 家族单独 lane，避免混入页面 destination 和站内写操作。
