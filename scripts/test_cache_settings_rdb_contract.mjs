...
const cachePolicy = read('shared/src/main/ets/settings/CachePolicy.ets')
...
for (const snippet of [
  ...
  assert(cacheSettings.includes(snippet), `CacheSettings missing RDB orchestration contract: ${snippet}`)
}

assert(!cacheSettings.includes("import { fileIo } from '@kit.CoreFileKit'"), 'CacheSettings must not import fileIo after payload extraction')
...
for (const snippet of [
  ...
  assert(cachePolicy.includes(snippet), `CachePolicy missing cache policy contract: ${snippet}`)
}
assert(!cachePolicy.includes('fileIo'), 'CachePolicy must not use fileIo')
assert(!cachePolicy.includes('relationalStore'), 'CachePolicy must not use relationalStore')
assert(!/^\s*(?!export const )[A-Za-z0-9_]+\(/m.test(cachePolicy), 'CachePolicy must contain constants only')

for (const snippet of [
  "import { fileIo } from '@kit.CoreFileKit'",
  'export interface PayloadStoragePlan',
  'export function preparePayloadStorage',
  'export function commitStagedPayloadFile',
  'export function cleanupPreparedPayloadFile',
  'export function resolvePayloadText',
  'export function deleteOrphanPayloadFiles',
  'export function unlinkPayloadFileBestEffort',
  ...
  assert(cachePayloadFiles.includes(snippet), `CachePayloadFiles missing payload contract: ${snippet}`)
}
for (const forbidden of ['relationalStore', 'preferences', 'ApiService', 'HttpClient', 'V2exNativeAuthService', 'pages/']) {
  assert(!cachePayloadFiles.includes(forbidden), `CachePayloadFiles must not import/use ${forbidden}`)
}
...
for (const policyContract of [
  ...
  },
  {
    ...
    pattern: /export\s+function\s+byteLength\s*\([\s\S]*charCodeAt[\s\S]*0xD800[\s\S]*0xDFFF[\s\S]*return bytes/,
  },
  {
    name: 'cache writes compute and store deterministic payload hash',
    source: cacheSettings,
    pattern: /private\s+static\s+async\s+saveCacheEntry[\s\S]*const nextPayloadHash = payloadHash\(payloadText\)[\s\S]*const args: relationalStore\.ValueType\[\] = \[[\s\S]*payloadPlan\.payloadPath,[\s\S]*nextPayloadHash,[\s\S]*payloadPlan\.size[\s\S]*\]/,
  },
  {
    name: 'cache reads verify non-empty payload hash after resolving payload and before parsing',
    source: cacheSettings,
    pattern: /private\s+static\s+async\s+loadCacheEntry[\s\S]*const storedPayloadHash = CacheSettings\.readNullableString\(resultSet,\s*hashIndex\)[\s\S]*const resolvedPayload = resolvePayloadText\(context,\s*payloadText,\s*payloadPath\)[\s\S]*if\s*\(resolvedPayload\.length <= 0\)[\s\S]*if\s*\(storedPayloadHash\.length > 0 && payloadHash\(resolvedPayload\) !== storedPayloadHash\)[\s\S]*await CacheSettings\.deleteCacheEntry\(context,\s*store,\s*cacheKey,\s*kind\)[\s\S]*return null[\s\S]*return\s*{[\s\S]*payloadText: resolvedPayload/,
  },
  {
    name: 'null or empty payload hash remains legacy-compatible',
    source: cacheSettings,
    pattern: /if\s*\(storedPayloadHash\.length > 0 && payloadHash\(resolvedPayload\) !== storedPayloadHash\)/,
  },
  {
    name: 'payload hash uses versioned FNV-1a UTF-8 byte checksum',
    source: cachePayloadFiles,
    pattern: /export\s+function\s+payloadHash[\s\S]*0x811c9dc5[\s\S]*v1:fnv1a32-utf8:[\s\S]*Math\.imul\(\(hash \^ \(byte & 0xFF\)\) >>> 0,\s*16777619\) >>> 0/,
  },
  {
    name: 'file-backed save stages payloads and stores only committed relative payload path',
    source: cachePayloadFiles,
    pattern: /export\s+function\s+preparePayloadStorage[\s\S]*size\s*<\s*FILE_PAYLOAD_THRESHOLD_BYTES[\s\S]*payloadText:\s*payloadText[\s\S]*payloadPath:\s*null[\s\S]*stagedPayloadPath:\s*null[\s\S]*committedPayloadFileName[\s\S]*stagedPayloadFileName[\s\S]*writePayloadFile\(context,\s*stagedName,\s*payloadText\)[\s\S]*payloadText:\s*null[\s\S]*payloadPath:\s*committedName[\s\S]*stagedPayloadPath:\s*stagedName/,
  },
  {
  ...
  },
  {
    ...
    source: cachePayloadFiles,
    pattern: /export\s+function\s+commitStagedPayloadFile[\s\S]*fileIo\.renameSync\([\s\S]*payloadPlan\.stagedPayloadPath[\s\S]*payloadPlan\.payloadPath[\s\S]*function\s+stagedPayloadFileName[\s\S]*return committedName \+ CACHE_PAYLOAD_STAGED_SUFFIX/,
  },
  {
  ...
  },
  {
    ...
    source: cachePayloadFiles,
    pattern: /export\s+function\s+resolvePayloadText[\s\S]*isCachePayloadFileName\(payloadPath\)[\s\S]*fileIo\.readTextSync[\s\S]*return payloadText/,
  },
  {
  ...
  },
  {
    ...
    source: cachePayloadFiles,
    pattern: /function\s+isCachePayloadFileName[\s\S]*!relativeName\.includes\(CACHE_PAYLOAD_STAGED_SUFFIX\)[\s\S]*function\s+isSafePayloadFileName[\s\S]*relativeName\.length\s*<=\s*0[\s\S]*relativeName\.startsWith\('\.'\)[\s\S]*relativeName\.includes\('\/'\)[\s\S]*relativeName\.includes\('\\\\'\)[\s\S]*relativeName\.includes\('\.\.'\)[\s\S]*\^\[A-Za-z0-9\._-\]\+/,
  },
]) {
  assert(policyContract.pattern.test(policyContract.source), `CacheSettings policy contract missing: ${policyContract.name}`)
}
