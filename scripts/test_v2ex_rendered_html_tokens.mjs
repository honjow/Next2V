...
const imageInternalsSource = readFileSync('shared/src/main/ets/components/markdown/MarkdownImageInternals.ets', 'utf8')
...
const settingsPageComponentsSource = readFileSync('feature/settings/src/main/ets/components/SettingsPageComponents.ets', 'utf8')
assert.match(source, /renderedHtmlToTokens/)
assert.match(source, /inlineHtmlToTokens/)
assert.match(imageRenderingSource, /function _inlineTokenKey\(token,? (?:Token|InlineImageTokenRecord), index,? number\)|function _inlineTokenKey\(token: (?:Token|InlineImageTokenRecord), index: number\): string/)
assert.match(source, /ForEach\(tokens, \(t: Token, tokenIndex: number\) => \{[\s\S]*?\}, \(t: Token, tokenIndex: number\) => _inlineTokenKey\(t, tokenIndex\)\)/)
...
const replyThreadCoordinatorSource = readFileSync('feature/detail/src/main/ets/model/ReplyThreadCoordinator.ets', 'utf8')
assert.match(replyThreadCoordinatorSource, /private static removeLeadingSingleMentionRendered\(contentRendered: string, users: string\[\], replyFloor: number\): string/)
assert.ok(replyThreadCoordinatorSource.includes('<p\\b[^>]*>'))
assert.match(replyThreadCoordinatorSource, /stripLeadingSingleMentionRenderedBody\(body, users\[0\], replyFloor\)/)
assert.match(replyThreadCoordinatorSource, /return ''[\s\S]*private static stripLeadingSingleMentionRenderedBody/)
...
assert.match(source, /return ReadingSettings\.scaleTypographyToken\(this\.headingBaseFontSize\(token\), this\.reading\.textScale\);/)
assert.match(source, /return ReadingSettings\.scaleTypographyToken\(this\.headingBaseLineHeight\(token\), this\.reading\.textScale\);/)
...
assert.match(settingsPageComponentsSource, /Text\((?:'文字缩放'|AppStrings\.R_TEXT_SCALE|\$r\('app\.string\.text_scale'\))\)/)
...
assert.match(readingSettingsPageSource, /将文字缩放恢复为默认值|R_RESTORE_READING_SCALE_MESSAGE|\$r\('app\.string\.restore_reading_scale_message'\)/)
...
assert.match(source, /const blockRe = \/<\(table\|h\[1-6\]\|p\|ul\|ol\|blockquote\|pre\|div\|object\)/)
...
assert.match(imageRenderingSource, /const INLINE_IMAGE_PENDING_SIZE = 24;/)
...
assert.match(inlineSizeSource, /contentMaxWidth <= 0[\s\S]*Math\.round\(widthPx\)[\s\S]*Math\.round\(heightPx\)/)
assert.match(inlineSizeSource, /scale = Math\.min\(1, contentMaxWidth \/ widthPx/)
assert.match(source, /@Local private paragraphAvailableWidth: number = 0;/)
