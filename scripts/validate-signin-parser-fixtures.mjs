...
assert.match(service, /return \[\{ key: challenge\.codeField \|\| 'code', value: cleanCode \}\]/)
assert.doesNotMatch(service, /pairs\.push\(\{ key, value: hiddenFields\[key\] \}\)/)
...
assert.match(service, /requestText\(`\$\{authBaseUrl\}\$\{V2exNativeAuthService\.SETTINGS_PATH\}`/)
...
assert.match(apiService, /getTextResponseWithHeaders\(endpoint, requestHeaders\)/)
assert.match(apiService, /assertNoTwoFactorChallenge\(endpoint, cookie, res\.text, res\.location\)/)
assert.match(apiService, /ApiService\.isLocationPath\(location, '\/2fa'\)[\s\S]*V2exSigninParser\.extractTwoFactorForm\(text\)/)
assert.match(apiService, /requestTwoFactorChallenge\(path: string, cookie: string, location: string\)/)
assert.match(apiService, /TwoFactorChallengeService\.request\(cleanCookie, path\)/)
...
const requestHtmlAssert = apiService.indexOf('this.assertNoTwoFactorChallenge(clean, cookie, text, location)', requestHtmlStart)
...
assert.match(page, /aboutToDisappear\(\): void \{\s*this\.pausePage\(\)\s*\}/s)
assert.match(page, /private pausePage\(\): void \{\s*this\.isPageActive = false[\s\S]*this\.challengeRequestId\+\+[\s\S]*this\.authRequestId\+\+/)
assert.match(page, /private clearSecretState\(\): void/)
assert.match(page, /@Local private captchaField: string = ''/)
assert.match(page, /private shouldShowCaptchaSection\(\): boolean \{\s*return this\.isChallengeLoading \|\| !!this\.captchaField \|\| !!this\.captchaUrl\s*\}/s)
...
assert.match(page, /const requestId = \+\+this\.authRequestId[\s\S]*this\.auth\s*\.loginWithChallenge\(challenge, cleanUsername, pwd, captchaCode\)[\s\S]*if \(!this\.isCurrentAuthRequest\(requestId\)\)/)
...
assert.match(prompt, /\$r\('app\.string\.two_factor_description'\)/)
...
assert.match(indexPage, /\.bindSheet\(\s*\$\$this\.twoFactorVisible,\s*this\.GlobalTwoFactorSheet\(\)/s)
...
assert.doesNotMatch(accountPage, /TwoFactorChallengeService\.request/)
assert.match(accountPage, /validateSessionWithCookie\(cookie\)/)
...
assert.doesNotMatch(myTopicsPage, /TwoFactorChallengeService\.request/)
