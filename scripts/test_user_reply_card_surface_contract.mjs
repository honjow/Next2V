...
assert.match(markdownContent, /@Prop contentSelectable: boolean = true;[\s\S]*onContentClick\?: \(\) => void/, 'MarkdownContent default must preserve selectable detail mode and expose ordinary-content callback')
...
assert.match(markdownContent, /struct SelectableInlineTokenFlow[\s\S]*@Prop contentSelectable: boolean = true;[\s\S]*if \(this\.contentSelectable\)[\s\S]*\.textSelectable\(this\.textSelectable\)[\s\S]*else[\s\S]*\.onClick\(\(\) => \{\s*_openContent\(this\.options\);\s*\}\)/, 'SelectableInlineTokenFlow must respect preview non-selectable ordinary content taps for base64 badge paths')
