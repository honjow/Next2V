...
assert.match(markdownTableStruct[0], /compressionCap = Math\.floor\(availableWidth \/ Math\.max\(1, natural\.length\) \* 1\.5\)[\s\S]*cappedTargets[\s\S]*readableMin\.map/, 'MarkdownTable uses measured fair-share compression cap instead of proportional long-prose domination')
...
assert.match(markdownTableStruct[0], /Row\(\)[\s\S]*\.width\(this\.tableContentWidth\(\)\)/, 'MarkdownTable rows use explicit width')
