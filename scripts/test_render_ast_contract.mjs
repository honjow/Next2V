...
assert.match(source, /type RenderInlineKind = "text" \| "link" \| "strong" \| "em" \| "del" \| "codespan" \| "image" \| "break" \| "base64Decode"/)
...
assert.doesNotMatch(markdownTableStruct[0], /@(?:State|Local) private columnWidths|measuredColumnWidth|updateColumnWidth|onSizeChange/, 'MarkdownTable ordinary layout must not use per-cell runtime natural measurement column widths')
assert.match(markdownTableStruct[0], /@Local private tableAvailableWidth: number = 0;/, 'MarkdownTable records measured available width')
...
assert.match(markdownTableStruct[0], /MarkdownInlineText\(\{[\s\S]*\.constraintSize\(\{ minWidth: this\.columnWidth\(columnIndex\), maxWidth: this\.columnWidth\(columnIndex\) \}\)/, 'MarkdownTable cells constrain min/max width to allocated column width')
...
assert.match(imageRenderingSource, /function _inlineImageRenderSize\(token: (?:Token|InlineImageTokenRecord), sizeRecords: InlineImageSizeRecord\[\], availableWidth: number\)/)
...
assert.match(settingsPageComponentsSource, /Text\(\$r\('app\.string\.text_scale'\)\)/)
assert.match(settingsPageComponentsSource, /Text\(\$r\('app\.string\.reading_preview_body'\)\)/)
...
assert.match(readingSettingsPageSource, /AppStrings\.text\(\$r\('app\.string\.restore_reading_scale_message'\)/)
